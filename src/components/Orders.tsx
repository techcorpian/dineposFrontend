          ...
          <div>
            ...
            <div>
              ...
              <div className="bg-gray-100 px-4 py-3 rounded-lg mb-4">
                {orderViewData.orderList.map((data, index) => (
              ...
              {orderViewData.razorpay_order_id && (
                <p className="flex justify-between text-gray-700"><strong>UPI Order Id</strong> {orderViewData.razorpay_order_id}</p>
              )}
              {orderViewData.razorpay_payment_id && (
                <p className="flex justify-between text-gray-700"><strong>UPI Payment Id</strong> {orderViewData.razorpay_payment_id}</p>
              )}
            </div>
          </div>
        </Modal>
          ...
          {orders.map((order, index) => (
            ...
            >
              <td
                ...
                } p-3 text-sm text-center text-blue-500 font-light`}
              >
                #{order.orderId}
              </td>
              <td
                className={`${
                  index % 2 === 0 ? "bg-gray-200" : ""
                } p-3 text-sm text-center text-gray-600 font-light`}
              >
              ...
              <td
                className={`${
                  index % 2 === 0 ? "bg-gray-200" : ""
                } p-3 text-sm text-center font-light`}
              >
                ${order.payableAmount}
              </td>
              <td
                className={`${
                  index % 2 === 0 ? "bg-gray-200" : ""
                } text-center text-sm p-3 font-light`}
              >
                {order.paymentMode}
              </td>
            </tr>
